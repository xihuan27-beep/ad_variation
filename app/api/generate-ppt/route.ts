...
import type { CampaignMeta, WorkItem } from '@/lib/campaign';

export async function POST(req: NextRequest): Promise<NextResponse> {
  try {
    const body = await req.json();
    const { meta, items } = body as { meta: CampaignMeta; items: WorkItem[] };

    if (!items?.length) {
      return NextResponse.json({ error: '작업 항목이 없습니다.' }, { status: 400 });
    }

    const pptBuffer = await generateGuidePpt(meta, items);
